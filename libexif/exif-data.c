		     ...
		     unsigned int ds)
{
	...
	if (!memcmp (d + headerOffset, "II", 2))
		data->priv->order = EXIF_BYTE_ORDER_INTEL;
	else if (!memcmp (d + headerOffset, "MM", 2))
	...
	/* Fixed value */
	if (exif_get_short (d + headerOffset + 2, data->priv->order) != 0x002a)
		return;

	/* IFD 0 offset */
	offset = exif_get_long (d + headerOffset + 4, data->priv->order);
	exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
		  "IFD 0 at %i.", (int) offset);

	/* ds is restricted to 16 bit above, so offset is restricted too, and offset+8 should not overflow. */
	if (offset > ds || offset + headerOffset + 2 > ds)
		return;

	/* Parse the actual exif data (usually offset 14 from start) */
	exif_data_load_data_content (data, EXIF_IFD_0, d + headerOffset, ds - headerOffset, offset, 0);

	/* IFD 1 offset */
	n = exif_get_short (d + headerOffset + offset, data->priv->order);
	/* offset < 2<<16, n is 16 bit at most, so this op will not overflow */
	if (offset + headerOffset + 2 + 12 * n + 4 > ds)
		return;

	offset = exif_get_long (d + headerOffset + offset + 2 + 12 * n, data->priv->order);
	if (offset) {
		exif_log (data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
			  "IFD 1 at %i.", (int) offset);

		/* Sanity check. ds is ensured to be above 6 above, offset is 16bit */
		if (offset > ds - headerOffset) {
			exif_log (data->priv->log, EXIF_LOG_CODE_CORRUPT_DATA,
				  "ExifData", "Bogus offset of IFD1.");
		} else {
		   exif_data_load_data_content (data, EXIF_IFD_1, d + headerOffset, ds - headerOffset, offset, 0);
		}
	}
	 ...
	 * to that data.
	 */
	// HACK: subtracting 6 from d is a segfault risk. But loaders skip over hardcoded 6 bytes of EXIF header,
	// so shuld be safe
	interpret_maker_note(data, d + headerOffset - 6, fullds + headerOffset - 6);
